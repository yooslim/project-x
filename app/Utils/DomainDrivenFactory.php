 ...
 * @method $this trashed()
 */
abstract class DomainDrivenFactory
{
    ...
    public function modelName()
    {
        $resolver = static::$modelNameResolver ?? function (self $factory) {
            $namespacedFactoryBasename = Str::replaceLast(
                'Factory', '', Str::beforeLast(get_class($factory), '\Database\Factories')
            );
    ...
    public static function resolveFactoryName(string $modelName)
    {
        $resolver = static::$factoryNameResolver ?? function (string $modelName) {
            $appNamespace = Str::beforeLast($modelName, '\\Models'); // Domains\Authentication
            $modelName = Str::after($modelName, 'Models\\');

            return $appNamespace . '\\Database\\Factories\\' .$modelName.'Factory';
        };
